...
router.post("/professors/create", async (req, res) => {
  try {
    ...
    console.log("f");
    res.status(200).send({ user: professor, token });
  } catch (e) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (course != null) {
        const courseStudentsPopulated = await course
...
router.post(
  "/professors/add-studentToCourse",
  authProfessor,
  async (req, res) => {
          ...
          if (beginningDate.getDay() === day) {
            const currentLesson = new Lesson({
              date: new Date().setDate(
                new Date().getDate() + numbersPassedSinceBeginning
              ),
              course: courseID,
              attendedStudents: [],
            });
...
router.get(
  "/professors/getStudentAppearences",
  authProfessor,
  async (req, res) => {
    const courseID = req.query.courseID;
    ...
    try {
      const course = await Course.findById(courseID);
      const coursePopulated = await course
        .populate("lessons.lesson")
        .execPopulate();
